...
///
/// A duration represents a span of time. Unlike `std::time::Duration` the
...
impl Duration {
    ...
    /// Create a new `Duration` from a whole number of milliseconds (`u64`), returning an error on overflow.
    ///
    ...
    /// Create a new `Duration` from a whole number of microseconds (`u64`), returning an error on overflow.
    ///
    /// *Note*: this will return an error on overflow.
    ...
    /// Create a new `Duration` from a whole number of seconds (u64), returning
    /// an error on overflow.
    ///
    /// Note: [`Duration::from_secs`] accepts a `u32` and cannot overflow.
    /// This method accepts a `u64` for cases where the number of seconds may
    /// be large.
    ...
    /// Returns the whole number of microseconds within the `Duration`.
    pub const fn as_micros(&self) -> u64 {
        self.ns / Self::MICROSECOND.as_nanos()
    }

    /// Returns the whole number of milliseconds within the `Duration`.
    ...
    /// Multiply this duration by a floating point value.
    ///
    /// Note: the result is truncated, not rounded. Negative values, NaN, and
    /// infinity will produce undefined but safe results (saturating cast to u64).
