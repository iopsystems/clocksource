...
///
/// Since `coarse::Duration` has second resolution, there are no sub-second
/// constructors. Use `precise::Duration` for sub-second precision.
#[repr(transparent)]
...
impl TryFrom<crate::precise::Duration> for Duration {
    type Error = TryFromError;

    fn try_from(other: crate::precise::Duration) -> Result<Self, Self::Error> {
        let secs = other.as_secs();
        if secs > u32::MAX as u64 {
            Err(TryFromError {
                kind: TryFromErrorKind::Overflow,
            })
        } else {
            Ok(Self::from_secs(secs as u32))
        }
    }
}
